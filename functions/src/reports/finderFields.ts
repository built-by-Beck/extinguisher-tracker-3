...
function mergeFinderFields(
  ...
  return {
    ...
    parentLocation:
      row.parentLocation || stringOrEmpty(extinguisher.parentLocation),
    ...
    manufactureYear:
      row.manufactureYear ?? numberOrNull(extinguisher.manufactureYear),
    expirationYear:
      row.expirationYear ?? numberOrNull(extinguisher.expirationYear),
    isExpired: row.isExpired || boolOrFalse(extinguisher.isExpired),
    lifecycleStatus:
      row.lifecycleStatus ||
      stringOrEmpty(extinguisher.lifecycleStatus) ||
      null,
    complianceStatus:
      row.complianceStatus ||
      stringOrEmpty(extinguisher.complianceStatus) ||
      null,
  };
}
...
export async function enrichFinderFields<T extends InspectionResultSource>(
  ...
  const ids = Array.from(
    new Set(
      rows.map((row) => stringOrEmpty(row.extinguisherId)).filter(Boolean),
    ),
  );
  ...
  return rows.map((row) => ({
    ...row,
    ...mergeFinderFields(
      toFinderFieldRow(row),
      extinguisherById.get(stringOrEmpty(row.extinguisherId)),
    ),
  }));
}

export function hasFinderFields(
  rows: InspectionResultSource[] | undefined,
): boolean {
  return (
    Array.isArray(rows) &&
    rows.every(
