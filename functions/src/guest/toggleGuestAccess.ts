...
export const toggleGuestAccess = onCall<
  ToggleGuestAccessInput,
...
>({ enforceAppCheck: false }, async (request) => {
  ...
  if (!featureFlags?.guestAccess) {
    throwFailedPrecondition(
      'Guest access is only available on Elite and Enterprise plans.',
    );
  }
  ...
  if (enabled) {
    ...
    if (expiresDate > maxFuture) {
      throwInvalidArgument(
        'expiresAt cannot be more than 365 days in the future.',
      );
    }
