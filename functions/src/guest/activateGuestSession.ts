...
export const activateGuestSession = onCall<
  ActivateGuestSessionInput,
  Promise<ActivateGuestSessionOutput>
>({ enforceAppCheck: false }, async (request) => {
  ...
  if (signInProvider !== 'anonymous') {
    throw new HttpsError(
      'permission-denied',
      'Only anonymous users can activate guest sessions.',
    );
  }
  ...
  if ('shareCode' in input && input.shareCode) {
    ...
    const guestAccess = orgData.guestAccess as
      | GuestAccessData
      | null
      | undefined;
    ...
    if (!guestAccess || !guestAccess.enabled) {
      throw new HttpsError(
        'not-found',
        'Guest access is not enabled for this organization.',
      );
    }
    ...
    throw new HttpsError(
      'invalid-argument',
      'Provide either { orgId, token } or { shareCode }.',
    );
  }
  ...
  if (existingMemberSnap.exists) {
    const existingData = existingMemberSnap.data() as {
      role: string;
      expiresAt: Timestamp;
    };
    ...
    throw new HttpsError(
      'already-exists',
      'A non-guest membership already exists for this user.',
    );
  }
