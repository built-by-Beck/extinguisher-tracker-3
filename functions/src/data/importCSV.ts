...
export const importExtinguishersCSV = onCall(async (request) => {
  ...
  // Seed pending inspections for all newly created extinguishers if an active workspace exists
  if (created.length > 0) {
    const activeWsSnap = await adminDb
      .collection(`org/${orgId}/workspaces`)
      .where('status', '==', 'active')
      .limit(1)
      .get();

    if (!activeWsSnap.empty) {
      const wsDoc = activeWsSnap.docs[0];
      const workspaceId = wsDoc.id;
      const inspRef = adminDb.collection(`org/${orgId}/inspections`);

      // Look up the newly created extinguisher docs by assetId to get their doc IDs + data
      let seededCount = 0;
      for (let i = 0; i < created.length; i += 30) {
        const assetBatch = created.slice(i, i + 30);
        const newExtSnap = await extRef
          .where('assetId', 'in', assetBatch)
          .where('deletedAt', '==', null)
          .get();

        let inspBatch = adminDb.batch();
        let inspBatchCount = 0;

        for (const extDoc of newExtSnap.docs) {
          const extData = extDoc.data();
          // Only seed standard category (matches createWorkspace behavior)
          if (extData.category !== 'standard') continue;

          const inspDocRef = inspRef.doc();
          inspBatch.set(inspDocRef, {
          ...
          if (inspBatchCount >= 499) {
            await inspBatch.commit();
            inspBatch = adminDb.batch();
            inspBatchCount = 0;
          }
        }
      ...
      // Update workspace stats to reflect the newly seeded inspections
      if (seededCount > 0) {
        await wsDoc.ref.update({
          'stats.total': FieldValue.increment(seededCount),
          'stats.pending': FieldValue.increment(seededCount),
