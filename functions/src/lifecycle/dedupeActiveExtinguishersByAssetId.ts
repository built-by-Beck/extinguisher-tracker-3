...
export const dedupeActiveExtinguishersByAssetId = onCall<
  DedupeInput,
  Promise<DedupeOutput>
>(async (request) => {
  ...
  for (const [, docs] of byAsset) {
    ...
    const sorted = [...docs].sort(
      (a, b) => createdMillis(b.data()) - createdMillis(a.data()),
    );
    const [, ...dupes] = sorted;
    toRetire.push(...dupes);
  }
  ...
  for (let i = 0; i < toRetire.length; i += 450) {
    ...
    for (const d of chunk) {
      batch.update(d.ref, {
        ...
        retiredBy: uid,
        retirementReason:
          'Duplicate active asset slot (auto-dedupe by asset number)',
        status: 'retired',
        isActive: false,
        updatedAt: serverTs,
      });
