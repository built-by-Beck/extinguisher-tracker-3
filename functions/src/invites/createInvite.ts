...
>({ enforceAppCheck: false }, async (request) => {
  ...
  if (
    !role ||
    !VALID_INVITE_ROLES.includes(role as (typeof VALID_INVITE_ROLES)[number])
  ) {
  ...
  const featureFlags = orgData.featureFlags as
  ...
  if (!hasTeamMembers) {
    throwFailedPrecondition(
      'Team members are only available on Elite and Enterprise plans.',
    );
  }
