...
Cloud Functions handle trusted backend workflows.
...
Firebase Storage stores uploaded and generated assets.
...
Represents an organization / tenant / customer account.

All operational data exists under org/{orgId}.

usr

Represents user profile metadata tied to Firebase Authentication.

Contains user-level metadata only, not operational organization data.

invite

Represents pending invitations before membership is accepted.
...
The system relies on these authoritative sources:
...
Owner imports or creates extinguisher inventory

Owner invites team members

Monthly workspace is created

Inspectors perform inspections in the field

Inspectors search or scan extinguisher records

Inspectors complete inspection checklist and attestation

Compliance and lifecycle state update automatically

Notifications and dashboards surface due or overdue items

Workspace is archived

Compliance reports are generated or exported as needed
