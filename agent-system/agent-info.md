...
## 2026-06-30 — Daily Review Bot launch-promo pricing build fix — built_by_Beck

**Task (SMALL review fix):** Daily Review Bot found frontend lint/build failures in launch-promo pricing display code after a pricing UI refactor.

**Fixes:**
- `src/lib/billingConfig.ts`: removed duplicate `LAUNCH_PROMO_DISCOUNT_FRACTION`; exported `getLaunchPromoCode`, `launchPromoMonthlyPrice`, and `getLaunchPromoPriceDisclaimer` for display-only promo helpers.
- `src/lib/marketingPlanPricing.ts`: imported the promo plan type, made promo pricing active only when a paid plan id is supplied, added `promoBadge` to the display contract, and preserved annual/monthly launch-promo copy.
- Removed stale `MarketingPlanPriceDisplay` imports and the dead `paidPlanPriceFields` helper; updated the Vitest mock for the pricing helper.
- Manual browser verification found that `MarketingPlanDetailPage` also needed to pass the paid plan id into `marketingPriceForInterval`; fixed so `/plans/pro` detail toggles show promo pricing like the public cards.

**Validation:** touched-file Prettier, `pnpm lint`, `pnpm build`, `pnpm test`, `npm --prefix functions run lint`, `npm --prefix functions run build`, `npm --prefix functions test` all pass. Manual browser walkthrough confirmed `/pricing` promo cards and Pro plan detail monthly/yearly toggle render without errors. `pnpm format:check` still fails on 38 pre-existing unrelated files (none touched by this fix).

**Open review finding:** `functions/src/billing/createCheckoutSession.ts` and `functions/src/billing/createPortalSession.ts` still build Stripe success/cancel/return URLs from `request.rawRequest?.headers?.origin` with a localhost fallback. This should be planned as a high-risk billing/security review item (allowlist configured app origins or canonical app URL), not changed casually in a daily review.
