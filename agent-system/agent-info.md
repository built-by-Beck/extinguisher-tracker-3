...
**Validation:** touched-file Prettier, `pnpm lint`, `pnpm build`, `pnpm test`, `npm --prefix functions run lint`, `npm --prefix functions run build`, `npm --prefix functions test` all pass. `pnpm format:check` still fails on 38 pre-existing unrelated files (none touched by this fix).
