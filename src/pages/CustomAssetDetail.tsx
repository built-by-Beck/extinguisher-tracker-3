...
import {
  GpsCapture,
  type GpsData,
} from '../components/inspection/GpsCapture.tsx';
...
export default function CustomAssetDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let activeInspection = await getInspectionForAssetInWorkspace(
        orgId,
        assetId,
        activeWorkspace.id,
      );
      if (
        !activeInspection &&
        canInspect &&
        loadedAsset.recurrence === 'monthly'
      ) {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to load custom asset.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function saveInspection() {
    ...
    const hasUnchecked = inspectionColumns.some(
    ...
    try {
      ...
      const overallStatus = Object.values(answers).some(
      ...
      <div className="p-6">
        <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-6">
          ...
          <p className="mt-2 text-sm text-gray-600">
            This Pro feature is locked for your current organization.
          </p>
          <Link
  ...
  return (
    <div className="p-6">
      <button
        onClick={() => navigate('/dashboard/custom-asset-inspections')}
        className="mb-4 flex items-center gap-1 text-sm text-gray-500 hover:text-gray-800"
      >
        <ArrowLeft className="h-4 w-4" />
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {workspace
                ? `${workspace.label} · ${statusLabel(inspection?.status ?? 'pending')}`
                : 'No active workspace for this month.'}
            </p>
          </div>
        ...
        {!workspace ? (
          <p className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-600">
            Create this month&apos;s workspace from the Inspections page before
            completing custom asset inspections.
          </p>
        ) : !inspection ? (
          <p className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-600">
            This asset is not available for the current workspace. Only active
            monthly custom assets are seeded for v1.
          </p>
        ) : (
          <>
                        ...
                        <tr key={item.id}>
                          ...
                          <td className="px-3 py-3">
                            <input
                              value={answer.notes ?? ''}
                              onChange={(event) =>
                                setAnswerNotes(item.id, event.target.value)
                              }
                              disabled={!canInspect || isCompleted}
              ...
              <aside className="space-y-4 rounded-xl border border-indigo-100 bg-indigo-50/40 p-4">
                <div className="rounded-lg border border-indigo-100 bg-white p-3">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    This asset row and its inspection columns are saved on the
                    custom asset. Next month&apos;s workspace will reuse them
                    automatically.
                  </p>
                </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-500">
          History uses the original inspection-column snapshot for each
          inspection.
        </p>
        <div className="mt-4 divide-y divide-gray-100">
          {history.filter(
            ...
            <p className="py-6 text-sm text-gray-500">
              No completed custom asset inspections yet.
            </p>
          ) : (
            history
              .filter(
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {item.inspectedByEmail ?? 'Unknown inspector'}
                      </p>
                    </div>
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === 'pass' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                    >
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="ml-2 text-gray-500">
                            {(answer?.result ?? 'unchecked').toUpperCase()}
                          </span>
                        </div>
                      );
                    })}
