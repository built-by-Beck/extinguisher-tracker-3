...
const COMPLIANCE_COLORS: Record<string, string> = {
  ...
  monthly_due: 'bg-yellow-100 text-yellow-700',
  annual_due: 'bg-orange-100 text-orange-700',
                  ...
                  <td className="whitespace-nowrap px-4 py-3">
                    <span
                      className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${
                        COMPLIANCE_COLORS[ext.complianceStatus ?? ''] ?? 'bg-gray-100 text-gray-600'
                      }`}
                    >
                      {(ext.complianceStatus ?? 'unknown').replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}
                    </span>
                  </td>
                  {canEdit && (
