...
const EXAMPLE_ROWS = [
  {
    ...
    extinguisherSize: '10 lbs',
    status: 'complete',
  },
  {
    assetId: 'EX-003',
    serial: '',
    parentLocation: 'Warehouse B',
    extinguisherType: 'ABC Dry Chemical',
    manufactureYear: '2019',
    extinguisherSize: '20 lbs',
    status: 'missing serial',
  },
  {
    assetId: 'EX-004',
    serial: 'SN-22222',
    ...
    extinguisherSize: '',
    status: 'missing location, type, size',
  },
  {
    assetId: 'EX-005',
    serial: 'SN-33333',
    parentLocation: 'Building C',
    extinguisherType: '',
    manufactureYear: '',
    extinguisherSize: '5 lbs',
    status: 'missing type, year',
  },
];
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            Data Organizer Guide
          </h1>
        </div>
        <p className="mt-2 text-gray-600">
          The Data Organizer helps you fix incomplete extinguisher records after
          an import. This guide explains what it checks, why each field matters,
          and how to format your data so everything lines up on the first try.
        </p>
      </div>
      ...
      <div className="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-gray-700">
          When you import extinguishers from a spreadsheet, some records may be
          missing key details like a location, serial number, type, or
          manufacture year. The Data Organizer scans your inventory and flags
          every extinguisher that has one or more missing fields. From there you
          can:
        </p>
        <ul className="mt-3 space-y-2 text-sm text-gray-700">
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong>Edit one at a time</strong> — click on any row and fill in
              the missing fields inline.
            </span>
          </li>
          <li className="flex items-start gap-2">
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-green-500" />
            <span>
              <strong>Bulk-assign locations</strong> — select multiple
              extinguishers and assign them all to the same building and section
              at once.
            </span>
          </li>
          <li className="flex items-start gap-2">
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-green-500" />
            <span>
              <strong>Filter by issue type</strong> — use the dropdown to show
              only extinguishers missing a specific field (e.g. "No serial" or
              "No type").
            </span>
          </li>
        </ul>
      ...
      <div className="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          If any of these fields are empty, the extinguisher will appear in the
          Data Organizer. Fill them in to get it off the list.
        </p>
      ...
      <div className="mb-8 rounded-xl border-2 border-dashed border-orange-200 bg-orange-50 p-6">
        ...
        <p className="mt-2 text-sm text-gray-600">
          This example CSV uses the exact column names the system expects. Open
          it in Excel or Google Sheets, replace the sample rows with your own
          data, and import it from the Inventory page. Records with complete
          data will pass right through; records with missing fields will show up
          in the Data Organizer for you to fix.
        </p>
        ...
        <p className="mt-3 text-xs text-gray-500">
          The example file includes 5 rows — some complete, some with missing
          fields on purpose so you can see exactly what triggers the Data
          Organizer.
        </p>
      </div>
      ...
      <div className="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Here is a preview of the example file. Notice that rows 3, 4, and 5
          have blank cells — those extinguishers would appear in the Data
          Organizer after import.
        </p>
            ...
            <tbody className="text-gray-600">
              {EXAMPLE_ROWS.map((row) => (
                <tr key={row.assetId} className="border-t border-gray-100">
                  ...
                  <td
                    className={`whitespace-nowrap px-3 py-2 font-mono text-xs ${!row.serial ? 'bg-red-50 text-red-400 italic' : ''}`}
                  >
                    {row.serial || '(empty)'}
                  </td>
                  <td
                    className={`whitespace-nowrap px-3 py-2 font-mono text-xs ${!row.parentLocation ? 'bg-red-50 text-red-400 italic' : ''}`}
                  >
                    {row.parentLocation || '(empty)'}
                  </td>
                  <td
                    className={`whitespace-nowrap px-3 py-2 font-mono text-xs ${!row.extinguisherType ? 'bg-red-50 text-red-400 italic' : ''}`}
                  >
                    {row.extinguisherType || '(empty)'}
                  </td>
                  <td
                    className={`whitespace-nowrap px-3 py-2 font-mono text-xs ${!row.manufactureYear ? 'bg-red-50 text-red-400 italic' : ''}`}
                  >
                    {row.manufactureYear || '(empty)'}
                  </td>
                  <td
                    className={`whitespace-nowrap px-3 py-2 font-mono text-xs ${!row.extinguisherSize ? 'bg-red-50 text-red-400 italic' : ''}`}
                  >
      ...
      <div className="mb-8 rounded-xl border border-blue-200 bg-blue-50 p-6">
        ...
        <p className="mt-2 text-sm text-gray-700">
          The import system recognizes many common variations. For example,{' '}
          <strong>&quot;Serial Number&quot;</strong>,{' '}
          <strong>&quot;SN&quot;</strong>, or{' '}
          <strong>&quot;serial&quot;</strong> all map to the same field. If a
          column can&apos;t be matched automatically, a Column Mapper will pop
          up and let you match it yourself. But using the exact column names
          from the example file is the fastest way to skip that step entirely.
        </p>
      </div>
      ...
      <div className="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <ol className="mt-4 space-y-4 text-sm text-gray-700">
          <li className="flex items-start gap-3">
            ...
            <span>
              <strong>Download the example</strong> above and use it as a
              starting point. Replace the sample rows with your real data,
              keeping the header row as-is.
            </span>
          </li>
          <li className="flex items-start gap-3">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700">
            ...
            <span>
              <strong>Fill in what you know.</strong> If you don&apos;t have a
              serial number or manufacture year for every extinguisher,
              that&apos;s fine — leave the cell blank and import anyway.
            </span>
          </li>
          <li className="flex items-start gap-3">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700">
          ...
          <li className="flex items-start gap-3">
            ...
            <span>
              ...
              to see which extinguishers have gaps. Fix them one by one or use
              bulk-assign for locations.
            </span>
          </li>
        </ol>
      </div>
