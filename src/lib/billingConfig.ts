...
export function launchPromoMonthlyPrice(monthlyPrice: number): number {
  const discounted = applyLaunchPromoDiscount(monthlyPrice);
  const snappedToCharmPrice = Math.floor(discounted) + 0.99;

  if (Math.abs(discounted - snappedToCharmPrice) <= 0.01) {
    return Math.round(snappedToCharmPrice * 100) / 100;
  }
...
export function getLaunchPromoPriceDisclaimer(
  ...
  const regularPrice =
    interval === 'year'
      ? `${formatUsd(Math.round(monthlyPrice * 12 * 0.9 * 100) / 100)}/yr`
      : `${formatUsd(monthlyPrice)}/mo`;

  return `50% off your first year with code ${code} at checkout. After 12 months, billing returns to ${regularPrice}.`;
}
