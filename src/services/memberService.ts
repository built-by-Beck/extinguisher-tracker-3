...
export async function callAcceptInvite(
  input: AcceptInviteInput,
): Promise<AcceptInviteOutput> {
  const fn = httpsCallable<AcceptInviteInput, AcceptInviteOutput>(
    functions,
    'acceptInvite',
  );
  const result = await fn(input);
...
export async function callChangeMemberRole(
  input: ChangeMemberRoleInput,
): Promise<void> {
  const fn = httpsCallable<ChangeMemberRoleInput, void>(
    functions,
    'changeMemberRole',
  );
  await fn(input);
...
export async function callRemoveMember(
