        ...
        {PLANS.filter((p) => p.name !== 'enterprise').map((plan) => {
          ...
          const priceDisplay =
            plan.monthlyPrice !== null
              ? marketingPriceForInterval(
                  plan.monthlyPrice,
                  billingInterval,
                  plan.name === 'enterprise' ? undefined : plan.name,
                )
              : null;
          const launchPromoCode =
            plan.name === 'enterprise' ? null : getLaunchPromoCode(plan.name);
          ...
          return (
            ...
            >
              ...
              {LAUNCH_PROMO_ENABLED && launchPromoCode ? (
                <p className="mt-2 text-xs font-semibold text-amber-800">
                  Code {launchPromoCode} at checkout
                </p>
              ) : null}
