...
async function parseExcelToRows(file: File): Promise<Record<string, string>[]> {
  const allRows = await readSheet(file);
  if (allRows.length === 0) return [];
  const [headerRow, ...dataRows] = allRows;
  const headers = headerRow.map(
    (h: string | number | boolean | typeof Date | null) =>
      h != null ? String(h) : '',
  );
...
export function ImportExportBar({ onImportJSON }: ImportExportBarProps) {
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      if (ext === 'xlsx') {
        ...
        setError(
          'Legacy .xls format is not supported. Please save the file as .xlsx and try again.',
        );
        return;
      } else if (ext === 'txt') {
  ...
  async function handleDrop(e: React.DragEvent) {
    ...
    if (!validExts.includes(ext)) {
      setError(
        `Unsupported file type: .${ext}. Use CSV, Excel (.xlsx), JSON, or TXT.`,
      );
      return;
    }
  ...
  async function performImport(
    ...
    if (defaultImportLocId) {
      ...
      if (loc) {
        ...
        if (!targetKeys.includes('parentLocation'))
          targetKeys.push('parentLocation');
  ...
  async function doImport(csvContent: string) {
    ...
    try {
      ...
      if (created > 0) {
        msg +=
          ' New items were added to inventory only; open an extinguisher to add it to the current month checklist.';
      }
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <div className="flex-1 rounded-lg border border-gray-200 bg-white p-3">
          ...
          >
            <FileSpreadsheet
              className={`mb-1.5 h-6 w-6 ${isDragOver ? 'text-red-400' : 'text-gray-400'}`}
            />
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              onChange={(e) => {
                setDefaultImportLocId(e.target.value);
                setError('');
              }}
              disabled={importing}
            ...
            <label
              className={`flex h-9 items-center gap-2 rounded-md border border-gray-300 px-3 text-sm font-medium ${
            ...
            >
              ...
              <input
                ...
                disabled={
                  importing || !defaultImportLocId || locations.length === 0
                }
                className="hidden"
              />
            </label>
