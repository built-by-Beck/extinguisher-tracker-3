          ...
          {scanning ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-2 text-gray-500">
                Your inventory is clean! All Asset IDs are unique.
              </p>
            </div>
          ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-amber-800">
                      Total extra records: {totalDuplicates}. Merging will
                      consolidate all data (photos, history, dates) into the
                      most complete record and remove the redundant ones.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="flex items-start gap-3 rounded-md bg-white p-3 border border-green-100 shadow-sm">
                        <span className="inline-flex items-center rounded-md bg-green-100 px-2 py-1 text-xs font-bold text-green-700">
                          KEEP
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 truncate">
                            ID: {group.keep.id?.slice(0, 8)}... •{' '}
                            {group.keep.category} •{' '}
                            {group.keep.complianceStatus}
                          </p>
                          <p className="mt-0.5 text-xs text-gray-500">
                            Last Inspection:{' '}
                            {formatDueDate(group.keep.lastMonthlyInspection)}
                          </p>
                        </div>
                      </div>
                      ...
                      {group.remove.map((rm) => (
                        <div
                          key={rm.id}
                          className="flex items-start gap-3 rounded-md bg-white p-3 border border-red-50 shadow-sm"
                        >
                          <span className="inline-flex items-center rounded-md bg-red-100 px-2 py-1 text-xs font-bold text-red-700">
                            REMOVE
                          </span>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">
                              ID: {rm.id?.slice(0, 8)}... • {rm.category} •{' '}
                              {rm.complianceStatus}
                            </p>
                            <p className="mt-0.5 text-xs text-red-500 italic">
                              Redundant record — will be merged and
                              soft-deleted.
                            </p>
                          </div>
                        </div>
