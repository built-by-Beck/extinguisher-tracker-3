...
export function ReplaceExtinguisherModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const serialInUse = await isSerialTaken(
        orgId,
        serial.trim(),
        oldExtinguisherId,
      );
      if (serialInUse) {
        setError(
          `Serial number "${serial.trim()}" is already in use by another active extinguisher.`,
        );
        setSubmitting(false);
        return;
      }
      ...
      if (barcode.trim()) {
        const bcTaken = await isBarcodeTaken(
          orgId,
          barcode.trim(),
          oldExtinguisherId,
        );
        if (bcTaken) {
          setError(
            `Barcode "${barcode.trim()}" is already in use by another active extinguisher.`,
          );
          setSubmitting(false);
          return;
        }
      ...
      const requestedAssetId =
        assetMode === 'replace' ? newAssetId.trim() : oldAssetId;
      if (!requestedAssetId) {
      ...
      if (assetMode === 'replace') {
        const assetInUse = await isAssetIdTaken(
          orgId,
          requestedAssetId,
          oldExtinguisherId,
        );
        if (assetInUse) {
          setError(
            `Asset ID "${requestedAssetId}" is already in use by another active extinguisher.`,
          );
          setSubmitting(false);
          return;
        }
      ...
      const my = manufactureYear.trim()
        ? parseInt(manufactureYear.trim(), 10)
        : null;
      const ey = expirationYear.trim()
        ? parseInt(expirationYear.trim(), 10)
        : null;
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to replace extinguisher.',
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="shrink-0 flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500">
              Confirm the old unit, then enter the new extinguisher information
              for this active record.
            </p>
          </div>
          <button
            type="button"
        ...
        >
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
            ...
            <div className="mt-3 grid gap-2 text-sm text-amber-950 sm:grid-cols-2">
              ...
              <p>
                Serial:{' '}
                <span className="font-mono font-medium">
                  {oldExtinguisher.serial || '—'}
                </span>
              </p>
              <p>
                Barcode:{' '}
                <span className="font-mono font-medium">
                  {oldExtinguisher.barcode || '—'}
                </span>
              </p>
              <p>
                Type:{' '}
                <span className="font-medium">
                  {oldExtinguisher.extinguisherType || '—'}
                </span>
              </p>
              <p>
                Size:{' '}
                <span className="font-medium">
                  {oldExtinguisher.extinguisherSize || '—'}
                </span>
              </p>
              <p>
                Manufacture year:{' '}
                <span className="font-medium">
                  {oldExtinguisher.manufactureYear ?? '—'}
                </span>
              </p>
              <p className="sm:col-span-2">
                ...
                <span className="font-medium">
                  {[
                    oldExtinguisher.parentLocation,
                    oldExtinguisher.section,
                    oldExtinguisher.vicinity,
                  ]
                    .filter(Boolean)
                    .join(' / ') || '—'}
                </span>
            ...
            <p className="mt-3 text-xs text-amber-800">
              This information will be archived automatically, so you do not
              need to copy old serial or barcode details into notes.
            </p>
          </div>
              ...
              <label className="flex items-start gap-3 rounded-lg border border-gray-200 bg-white px-3 py-3 text-sm">
                ...
                <span>
                  <span className="block font-medium text-gray-900">
                    Reuse existing asset/location ID
                  </span>
                  <span className="block text-gray-500">
                    Keep <span className="font-mono">{oldAssetId}</span> tied to
                    the location and move its barcode to the new extinguisher.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3 rounded-lg border border-gray-200 bg-white px-3 py-3 text-sm">
                ...
                <span className="flex-1">
                  <span className="block font-medium text-gray-900">
                    Replace asset ID number
                  </span>
                  <span className="block text-gray-500">
                    Use this when the location/asset barcode also changes.
                  </span>
                  {assetMode === 'replace' && (
                    <input
                      type="text"
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <p className="mb-4 text-sm text-gray-500">
              Check these only when the replacement unit was just serviced. This
              resets the next due date from today.
            </p>
            <div className="space-y-3">
              <label className="flex items-start gap-3 rounded-lg border border-gray-200 bg-white px-3 py-3 text-sm">
                <input
