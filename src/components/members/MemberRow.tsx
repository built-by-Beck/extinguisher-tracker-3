...
export function MemberRow({
  ...
  return (
    <tr
      className={`border-b border-gray-100 ${isCurrentUser ? 'bg-blue-50/50' : ''}`}
    >
      ...
      <td className="px-4 py-3">
        <span
          className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${roleBadgeStyles[member.role]}`}
        >
      ...
      <td className="px-4 py-3">
        <span
          className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${statusBadgeStyles[member.status] ?? 'bg-gray-100 text-gray-700'}`}
        >
          {member.status}
        </span>
      </td>
